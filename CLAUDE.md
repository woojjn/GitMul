...
#### Phase 4: Image Diff (0% - 다음 작업)
- **목표**: 이미지 파일 변경 시각화
- **필요 작업**:
  1. 이미지 파일 감지 로직 추가 (확장자: .png, .jpg, .gif, .svg, .webp)
  2. `ImageDiff.tsx` 컴포넌트 생성
     - Side-by-side 레이아웃
     - Old/New 이미지 표시
     - 메타데이터: 크기, 해상도, 포맷
  3. `DiffViewer.tsx`에 조건부 렌더링 추가
  4. 백엔드: 이미지 바이너리 읽기 (`src-tauri/src/commands/diff.rs`)
  5. Base64 인코딩으로 프론트엔드 전달

**예상 소요 시간**: 1.5~2시간

**구현 가이드**:
```typescript
// DiffViewer.tsx에서 이미지 감지
const isImage = (path: string) => /\.(png|jpe?g|gif|svg|webp)$/i.test(path);

if (isImage(filePath)) {
  return <ImageDiff oldPath={...} newPath={...} />;
}
```
...
### 현재 제한사항
1. **이미지 Diff 미지원** (Phase 4에서 구현 예정)
...
# Phase 4 시작하기
# 1. src/components/ImageDiff.tsx 생성
# 2. src/components/DiffViewer.tsx 수정
# 3. src-tauri/src/commands/diff.rs 수정
```
